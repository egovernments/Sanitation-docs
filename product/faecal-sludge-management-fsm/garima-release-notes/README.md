...
*   Introduction of a new column of string type (boundary\_type) in the vehicle\_trip table to capture if the vehicle log is created for the GP or not.
...
* To get a list of gram panchayats and corresponding villages, pass the boundaryType as 'GP'. Below are the API details:\
  [https://dev.digit.org/egov-location/location/v11/boundarys/\_search?hierarchyTypeCode=REVENUE&](https://dev.digit.org/egov-location/location/v11/boundarys/\_search?hierarchyTypeCode=REVENUE&)**boundaryType=GP**\&tenantId=pb.amritsar
...
| -------------------- | ------------------------------- | ---------------------------------------------------------------------------------------------------------------------------- |
| eg\_fsm\_application | boundarytype                    | To capture (locality or GP) to determine if the application created is for rural or not based on the radio button selection. |
...
**For FSM Module:**\
Non-URC flow: The locality drop-down should show all the localities as it is (there should be no impact).\
URC flow: The gram panchayat and village drop-down should show as per the new GP and village added to the MDMS.

**For Property Tax Module:**\
The locality drop-down should show all the localities as it is (there should be no impact).
...
### **Employee Flow**  <a href="#employee-flow" id="employee-flow"></a>
...
*   GP drop-down\
    In the case of FSM (URC), the gram panchayat and village drop-downs are showing values from the new hierarchy we have configured in the existing MDMS.\
...
### **FSTP Flow**  <a href="#fstp-flow" id="fstp-flow"></a>
...
### Citizen flow <a href="#citizen-flow" id="citizen-flow"></a>
